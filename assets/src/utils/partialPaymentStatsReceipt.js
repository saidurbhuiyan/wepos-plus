...
async function uploadPDF(pdfBlob, filename) {
	...
	try {
		const response = await fetch(
			weposData.mediaUrl + '?search=' + filename,
			{
				method: 'GET',
				headers: {
...
async function generateReceiptPDF(printdata, settings, partialPaymentId,actionType = 'generate-receipt', phoneNumber = null) {
	...
	if (printdata.coupon_lines && printdata.coupon_lines.length > 0) {
		...
		if (fixedDiscount) {
			...
			printdata.coupon_lines.forEach(
				fee => {
					doc.text( `Discount:`, 11, yPosition );
	...
	if (printdata.refund > 0) {
		...
		doc.text( formatPrice( printdata.refund ), 198, yPosition, {align: 'right'} );
	}
	...
	if (printdata.total_refund > 0) {
		...
		doc.text( formatPrice( printdata.total_refund ), 198, yPosition, {align: 'right'} );
	...
	function($) {
		$( '.partial-payment-stats' ).on(
			...
			async function(e) {
				...
				const refund                        = parseFloat( partialReceipt.attr( 'data-partial-refund' ) ?? 0 ).toFixed( 2 );
				const totalRefund                   = parseFloat( partialReceipt.attr( 'data-partial-total-refund' ) ?? 0 ).toFixed( 2 );
